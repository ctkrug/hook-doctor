...
import { SENTENCE_VARIANCE_ID, sentenceVariance } from "./rules/sentenceVariance";
...
export function computeHookScore(rules: RuleResult[]): number {
  const total = rules.reduce((sum, rule) => sum + rule.score * (WEIGHTS[rule.id] ?? 0), 0);
...
export function diagnose(input: string): Diagnosis {
  const rules = [inMediasRes(input), imageryDensity(input), sentenceVariance(input)];
  return {
    input,
    rules,
